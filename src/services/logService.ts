...
export class LogService {
    ...
    async write(logLevel: LogLevel, component: string, process: string, message: string,
        ...
        if (!!this.settings.RippleApi &&
            !!this.settings.RippleApi.LogAdapterUrl) {
            try {
                await axios.post(this.settings.RippleApi.LogAdapterUrl, {
                    appName: APP_NAME,
                    ...
                    callstack: stack,
                    exceptionType: type,
                    additionalSlackChannels: this.settings.RippleApi.LogSlackChannels
                });
            } catch (err) {
